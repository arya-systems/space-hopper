export default class CPReduce {
  ...
  static reduce(body, jd_utc, observer) {
    ...
    let observerGeocentric = this.getObserverGeocentricPosition(
      observer,
      jd_ut1,
    );
    observerGeocentric = Vec.vecMatrixMul(
      observerGeocentric,
      Vec.transpose(nutationMatrix),
    );
    observerGeocentric = Vec.vecMatrixMul(
      observerGeocentric,
      Vec.transpose(precessionMatrix),
    );
  ...
  static getObserverGeocentricPosition(observer, jd_ut) {
    const ecef = this.convertGeodedicLatLonToECEFXYZ(
      observer[0],
      observer[1],
      observer[2],
    );
    ...
    const C =
      ...
      Math.sqrt(
        Math.cos(lat) * Math.cos(lat) +
          (1.0 - f) * (1.0 - f) * (Math.sin(lat) * Math.sin(lat)),
      );
  ...
  static earthRotationAngle(jd_ut1) {
    ...
    let era =
      (Math.PI * 2 * (0.779057273264 + 0.00273781191135448 * t + frac)) %
      (Math.PI * 2);
...
const CprSun = (CPReduce.SUN = 0);
const CprMercury = (CPReduce.MERCURY = 1);
const CprVenus = (CPReduce.VENUS = 2);
const CprEarth = (CPReduce.EARTH = 3);
const CprMars = (CPReduce.MARS = 4);
const CprJupiter = (CPReduce.JUPITER = 5);
const CprSaturn = (CPReduce.SATURN = 6);
const CprUranus = (CPReduce.URANUS = 7);
const CprNeptune = (CPReduce.NEPTUNE = 8);
const CprEmb = (CPReduce.EMB = 9);
const CprMoon = (CPReduce.MOON = 10);
const CprBodies = (CPReduce.bodies = [
  "Sun",
  "Mercury",
  "Venus",
  "Earth",
